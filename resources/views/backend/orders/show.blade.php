               ...
               <div class="col-md-12 h-line">
                  <div>
                    <div class="pull-left m-t-30">
                        ...
                        @if($order->status == 1)
                        <span class="label stick-show change-status" data-status="{!! $order->status !!}" data-id="{{$order->id}}" style="background-color:#777777">Đang chờ</span></p>
                        @endif
                        @if($order->status == 2)
                        <span class="label stick-show change-status" data-status="{!! $order->status !!}" data-id="{{$order->id}}"  style="background-color:#D9534F">Bị hủy</span></p>
                        @endif
                        @if($order->status == 3)
                        <span class="label stick-show change-status" data-status="{!! $order->status !!}" data-id="{{$order->id}}"  style="background-color:#0CC2AA">Đang xử lý</span></p>
                        @endif
                        @if($order->status == 4)
                        <span class="label stick-show change-status" data-status="{!! $order->status !!}" data-id="{{$order->id}}"  style="background-color:#F0AD4E">Đang giao hàng</span></p>
                        @endif
                        @if($order->status == 5)
                        <span class="label stick-show change-status" data-status="{!! $order->status !!}" data-id="{{$order->id}}"  style="background-color:#5CB85C">Đã thanh toán</span></p>
                        @endif
                        @if($order->status == 6)
                        <span class="label stick-show change-status" data-status="{!! $order->status !!}" data-id="{{$order->id}}"  style="background-color:#337AB7">Đã nhận hàng</span></p>
                        @endif
                    </div>
                    <div class="clearfix"></div>
                  </div>
                      ...
                      <table class="table m-t-30">
                          ...
                          <tbody>
                              @foreach ($order->getItem as $key => $item)
                                ...
                                $percen = App\Customer::where('phone',$order->phone)->first();
                                if($percen){
                                     $config = App\Configure_discounts::get();
                                      $percent_x = 0;
                                      foreach ($config as $key => $value) {
                                        if($value->targets <= $percen->points ){
                                            $percent_x = $value->percent;
                                        }
                                      }
                              ...
                              <tr style="background-color: rgba(0, 0, 0, 0) !important;"> 
                                  ...
                                  @if($percen)
                                    <tr style="background-color: rgba(0, 0, 0, 0) !important;"> 
                                          ...
                                          <td colspan="2">
                                            <div class="col-md-12" style="padding:0px;">
                                                <span class="text-right"> <h6 style="font-size:14px !important;">Giảm {{ $percent_x }} %: -{!! number_format((int)$b,0,'','.') !!} đ</h6> </span>
                                            </div>
                                          </td>
                                    </tr>
                                  @endif
                              ...
                              <tr style="background-color: rgba(0, 0, 0, 0) !important;"> 
                                    ...
                                    <td colspan="2">
                                      <div class="col-md-12" style="padding:0px;">
                                          <span class="text-right"> @if(!$percen) <h6> Tổng đơn hàng: {{number_format($order->total + $order->total_weight, 0, '', '.')}} đ </h6> @else <h6> Tổng đơn hàng: {{number_format(($order->total + $order->total_weight)-$b, 0, '', '.')}} đ </h6> @endif</span>
                                          {{-- <span class="text-right"> <h6> Tổng đơn hàng: {{number_format($order->total, 0, '', '.')}} đ </h6></span> --}}
                                      </div>
                                    </td>
                              </tr>
                          </tbody>
...
{Domain}</textarea>
                   ...
                   <div class="col-sm-4">
                       <div class="email-tb">
                           <div class="wrap-email-tb">
                               <p>Email thông báo đang xử lí</p>
                               <p>Tiêu đề email</p>
                               <input name="description_bihuy" type="text" value="{Domain} Thông báo đơn hàng đang xử lí {mã đơn hàng}" class="form-control" disabled >
                               <p>Nội dung email</p>
                               <textarea name="content_bihuy" cols="20" rows="4" disabled class="form-control">Đơn hàng của bạn đang được xử lí
Mã đơn hàng : {Mã đơn hàng}
{Domain}</textarea>
                           </div>
                       </div>
