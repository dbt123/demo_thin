...
<?php
  if($stt==1){
   $comments = App\CommentFrame::select('comment_frames.*','accounts.name as acc_name','frames.name')->leftJoin('accounts', 'comment_frames.account_id', '=', 'accounts.id')->where('comment_frames.status',1)->leftJoin('frames', 'frames.id', '=', 'comment_frames.frame_id')->orderby('created_at','desc')->paginate(10); 
  }
?>
