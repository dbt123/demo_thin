...
class OrderController extends Controller
{
    ...
    public function postOrderadd(Request $req){
        $l_p = $req->productId;
        if($l_p == null){
            // return redirect()->back()->with('error','Vui lòng chọn Sản Phẩm cần thêm');
                ...
                if($id_d <= 0){
                    ...
                    if($order->save()){
                        ...
                        if($size > 0){
                            ...
                            $config_price = Configure_discounts::get();
                            $percen = 0;
                            foreach ($config_price as $key => $value){
                                if($value->targets <= $sum){
                                    $percen = $value->percent;
                                }else{
                                    break;
                                }
                            }
                            $order->percent = (float)$percen;
                        ...
                        $mail = System::select('email_send','email_password')->first();
                         if (  $email_rev ) {
                            ...
                            if($email_rev->email){
                                    ...
                                    $message->to($email_rev->email, 'Quản trị')->subject(" Có đơn hàng mới "."#".$order->id);
                                    $message->from($mail->email_send);
                                    $message->to($order->email, 'Quản trị')->subject(" Có đơn hàng mới "."#".$order->id);
                                });
                            }
                        }
                        $link = route('order.show',['id'=>$order->id]);
        ...
        if(sizeof($order)){
            ...
            if($order->save()){
                                    ...
                                    try {
                                        ...
                                        if ( !empty($mail->email_send) && !empty($mail->email_password) ) {
                                            ...
                                            if($order->email){
                                                    ...
                                                    $message->from($mail->email_send);
                                                    $message->to($order->email, 'QTV')->subject(url('')." Thông báo đơn hàng bị hủy"." #".$order->id);
                                                });
                                            }
                                        }  
                                    }catch (Exception $e) { 
                    ...
                    try {
                        ...
                        if ( !empty($mail->email_send) && !empty($mail->email_password) ) {
                            ...
                            if($order->email){
                                    Mail::send('backend.email.email-bi-huy',$data, function($message) use ($mail,$order){
                                    $message->from($mail->email_send);
                                    $message->to($order->email, 'QTV')->subject(url('')." Thông báo đơn hàng đang xử lí"." #".$order->id);
                                });
                            }
                        }  
                    }catch (Exception $e) { 
                ...
                if($order->status == 5 && $last_status != 5){
                   ...
                   // $mail_thanhtoan = Email_General::where('name','Config Email Thanh Toán')->first();
